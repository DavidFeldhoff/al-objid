...
async function getTypeAtPositionRaw(
    ...
    if (
        ...
        if (objectParts[1] !== "0") {
            ...
            if (Config.instance.fieldAndValueIdsStayInsideExtensionRange && matches[0].name.toLowerCase().startsWith("tableextension")) {
    ...
    if (matches.length > 0 && matches[0].name.toLowerCase().startsWith("enum")) {
        ...
        if (isValue) {
            if (objectParts[1] !== "0") {
                ...
                if (Config.instance.fieldAndValueIdsStayInsideExtensionRange && matches[0].name.toLowerCase().startsWith("enumextension")) {
                    const extendedId = await getExtendedId(document.uri, document.getText(), document.eol)
                    if (extendedId) {
                        return `${objectParts[0]}_${extendedId}`;
                    }
